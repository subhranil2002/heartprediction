...
app.config["MAIL_USERNAME"] = "pratyushjagdishbirole2022@vitbhopal.ac.in"
app.config["MAIL_PASSWORD"] = "pratyushvit" #enter your email password here, mine is hidden for privacy purposes
...
def check():
	if request.method == "POST":
		...
		with open("heartdiseaseprediction.model", "rb") as f:
			model = pickle.load(f)	
		res = model.predict(d)
		return render_template("find.html", msg = res)	
	else:
...
def signup():
	if request.method == "POST":
		...
		try:
			...
			cursor = con.cursor()
			con.execute("CREATE TABLE user (username TEXT UNIQUE, password TEXT)")
